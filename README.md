...
The source report is https://cs.nyu.edu/media/publications/TR1995-711.pdf
(cached in repo as [`sources/TR1995-711.pdf`](sources/TR1995-711.pdf)); its
draft vision transcription is
[`sources/TR1995-711_vision.md`](sources/TR1995-711_vision.md). The new paper
is [`arxiv.md`](arxiv.md). Figure 1 in `arxiv.md` is generated by
`python3 scripts/plot_nose.py` → [`figures/nose.png`](figures/nose.png). For
GitHub math rendering, run `python3 scripts/fix_github_math.py` after editing
equations.
...
The project targets Lean/mathlib 4.33.1 and contains no `sorry`. The former
global-completeness placeholder was replaced by the checked counterexample
`MLS.not_decideMLSSat_complete`: the current partial parser rejects a
satisfiable disjunction, so a full FOS80 algorithm must be implemented before
global completeness can be stated. The unfinished NBH and collapse routes now
take their compiler, universal-reduction, and collapse principles as explicit
parameters; the library contains no project-specific axioms.
...
Related formalization: Lukas Stevens's
[MLSS Decision Procedure](https://isa-afp.org/entries/MLSS_Decision_Proc.html)
(AFP, 2023) and
[CADE 29 paper](https://doi.org/10.1007/978-3-031-38499-8_28) verify in
Isabelle/HOL an executable, terminating, sound, and complete tableau procedure
for MLSS. MLSS adds singleton terms and is richer than this repository's MLS;
the tableau procedure also differs from the partial FOS80 model-graph
implementation here.

The local mechanical preflight passes. The earlier six-declaration surface was
rejected for low notability. The current surface's first review instead
scores literature notability 4/4 and is `revision_required` for documentation
alignment and related-work corrections. Generated reports remain under
`.cache/palomar-editorial/`.
