...
| Theorem 4.2 | A linear-time ranking corresponds to infinitely many `μ` for which `(SAT,μ)` is NP-distributional complete; hence SAT is NP-average complete. | `sources/RS93_vision.md`, `sources/Lev86_vision.md` | `Completeness.nbhProb_NPDistributionallyComplete` | Axiom-backed and not SAT; current NBH has a singleton distribution |
...
| Corollary 5.1 | MLS satisfiability is NP-average complete. | `sources/TR1995-711_vision.md` | `Completeness.satMLSProb_NPDistributionallyComplete` | Axiom-backed and stated for checker/singleton distribution rather than paper language |
...
| Theorem 5.2 | EMLS satisfiability is NP-complete and NP-average complete via an injective, honest, polynomial-time invertible SAT reduction. | `sources/TR1995-711_vision.md` | `EMLSReduction.satisfiable_iff`, `semanticCore_length` | Concrete elementary-literal semantic reduction correctness and exact linear conjunct count proved; injectivity/inversion with a size-accounted serialization, NP membership, and average-completeness transfer remain |
