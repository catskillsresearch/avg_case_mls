...
for key in ("challenge_module", "solution_module", "theorem_names",
...
for imp in imports:
    if imp.startswith("AvgCaseMls") or imp.startswith("Solution"):
...
step "Reject proof holes in selected Solution sources"
if rg -n --glob '*.lean' \
    '(^|:=|by)[[:space:]]+sorry([[:space:];]|$)|^[[:space:]]*sorry([[:space:];]|$)' \
    AvgCaseMls/Palomar.lean Solution.lean; then
  echo "FAIL: selected Solution proof sources contain sorry."
  exit 1
fi
echo "OK: selected Solution proof sources contain no sorry."
...
python3 scripts/palomar_mechanical_report.py \
  --out .cache/palomar-editorial/mechanical-report.json
