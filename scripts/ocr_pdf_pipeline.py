...
Usage (from repo root):
  ...
  bash scripts/ocr_pdf_pipeline.sh TR1995-711.pdf --pages 1-3
  bash scripts/ocr_pdf_pipeline.sh TR1995-711.pdf --png-only
  bash scripts/ocr_pdf_pipeline.sh TR1995-711.pdf --merge-only
  bash scripts/ocr_pdf_pipeline.sh TR1995-711.pdf --status
"""
...
def normalize_page_text(text: str) -> str:
    """Normalize a merged page without paper-specific content removal."""
    return text.strip() + "\n"
...
"""
    ...
    for n in stitch_pages:
        ...
        if not merged.exists():
            ...
            continue
        parts.append(f"\n<!-- page {n} -->\n\n")
        parts.append(normalize_page_text(merged.read_text(encoding="utf-8")))
...
def main() -> None:
    ...
    ap.add_argument(
        ...
        type=Path,
        nargs="?",
        default=Path("TR1995-711.pdf"),
        help="PDF name or path (default: TR1995-711.pdf under sources/)",
    )
